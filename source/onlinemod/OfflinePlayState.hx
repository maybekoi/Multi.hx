...
class OfflinePlayState extends PlayState
{
  ...
  override function create()
  {
	...
	boyfriend = new Boyfriend(770, 450, (!isStoryMode && freeplayChar ? selectedBF : PlayState.SONG.player1));
	...
	DiscordClient.changePresence("Rapping offline...", null);
	#end

    // Add XieneDev watermark
    var xieneDevWatermark:FlxText = new FlxText(-4, FlxG.height * 0.9 + 50, FlxG.width, "Multi.hx BETA ((OFFLINE MODE))", 16);
  ...
  {
    if (Type.getClass(SubState) == OfflinerGameOverSubstate)
    {
      super.openSubState(new OfflinerGameOverSubstate(boyfriend.getScreenPosition().x, boyfriend.getScreenPosition().y));
      return;
    }
